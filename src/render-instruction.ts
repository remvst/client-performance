...
export type PerformanceRecordRenderer = (current: FrameRecord, previous: FrameRecord) => number;
...
export function renderFramerate(event: string): PerformanceRecordRenderer {
    return function(
        ...
        const interval = (current.onStartTimes.get(event) || 0) - (previous.onStartTimes.get(event) || 0);
...
export function renderExecutionTime(event: string): PerformanceRecordRenderer {
    return function(
        ...
        return (current.onEndTimes.get(event) || 0) - (current.onStartTimes.get(event) || 0);
    }
}